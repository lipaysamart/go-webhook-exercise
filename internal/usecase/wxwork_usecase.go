...
const (
	WxworkWebHookURL = "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=70ef447c-99c4-4432-b916-ccdcc10057cf"
)
...
func (u *wxworkUsecase) Receive(ctx context.Context, payload map[string]interface{}) error {
	...
	if err := json.NewDecoder(&buf).Decode(&payload); err != nil {
		u.Logger.Trace().Ctx(ctx).Any("payload", payload).Msg("failed to decode payload")
		return err
	}
